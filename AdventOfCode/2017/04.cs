...
");
            ...
            {
                var pass = row.Split(" ").AsEnumerable().Select(_ => _);
                if (pass.Distinct().Count() == pass.Count())
                    count++;
            }
...
");
            ...
            foreach (var row in rows)
            {
                var pass = row.Split(" ").AsEnumerable().Select(_ => { var s = _.ToCharArray(); Array.Sort(s); return new string(s); });
                if (pass.Distinct().Count() == pass.Count())
                    count++;
            }

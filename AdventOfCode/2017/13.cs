...
6: 4";
*/
            IEnumerable<Layer> layers = input.Split("\n").Select(_ => { var separator = _.Split(": "); return new Layer() { Deep = int.Parse(separator[0]), Range = int.Parse(separator[1]) }; });
            int severity = 0;
            
            for(var t=0; t<=layers.Max(_ => _.Deep);t++)
            {
                var layer = layers.SingleOrDefault(_ => _.Deep == t);
                if (layer != null)
                    severity += t % (2*(layer.Range-1)) == 0 ? t * layer.Range : 0;
            }
...
You can't control the speed of the packet, but you can delay it any number of picoseconds. For each picosecond you delay the packet before beginning your trip, all security scanners move one step. 
...
What is the fewest number of picoseconds that you need to delay the packet to pass through the firewall without being caught?         
        ...
        public override string Output2(string input)
        {
            ...
            {
                ++delay;
                if (layers.All(_ => (_.Deep + delay) % (2 * (_.Range - 1)) != 0)) break;
            }

...
How many groups are there in total?         
        ...
        private void links(int i, IList<int> group)
        {
            if (group.Contains(i)) return;
            group.Add(i);
            foreach (int n in programs[i].Split(",").Select(_ => int.Parse(_.Trim())))
                links(n, group);
            programs[i] = null;
        }
    }

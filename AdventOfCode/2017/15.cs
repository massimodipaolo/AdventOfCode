...
After 40 million pairs, what is the judge's final count?
        */
        public override string Output(string input)
        {
            ...
            var genA = new generator() { value = int.Parse(starts[0]), factor = 16807 };
            var genB = new generator() { value = int.Parse(starts[1]), factor = 48271 };
            int match = 0;
            for (var i = 0; i < 40000000; i++)
                if (genA.next() == genB.next()) match++; ;
...
After 5 million pairs, but using this new generator logic, what is the judge's final count?
        ...
        */
        public override string Output2(string input)
        {
            //input = "65,8921";
            var starts = input.Split(',');
            var genA = new generator() { value = int.Parse(starts[0]), factor = 16807, module = 4 };
            var genB = new generator() { value = int.Parse(starts[1]), factor = 48271, module = 8 };
            int match = 0;
            for (var i = 0; i < 5000000; i++)
                if (genA.next2() == genB.next2()) match++; ;
        ...
        public class generator
        {
            ...
            public string next()
            {
                ...
                return new string(Convert.ToString(value, 2).PadRight(16, '0').Reverse().Take(16).Reverse().ToArray());
            }
            public string next2()
            {
                while (true)

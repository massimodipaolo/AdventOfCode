...
What is the checksum for the spreadsheet in your puzzle input?
        */
        ...
        public override string Output2(string input)
        {
            ...
            foreach (var row in input.Split('\r'))
            {
                var nums = row.Split(' ').AsEnumerable().Where(s => !string.IsNullOrEmpty(s)).Select(_ => int.Parse(_.ToString())).OrderBy(_ => _);
                foreach (int num in nums)
                    sum += nums.Where(_ => _ > num && _ % num == 0).Select(_ => _ / num).Sum();
            }
            return sum.ToString();
        }

...
This spinlock's algorithm is simple but efficient, quickly consuming everything in its path. It starts with a circular buffer containing only the value 0, which it marks as the current position. 
It then steps forward through the circular buffer some number of steps (your puzzle input) before inserting the first new value, 1, after the value it stopped on. The inserted value becomes the current position. 
...
What is the value after 2017 in your completed circular buffer?         
        ...
        {
            //input = "3";
            int spin = int.Parse(input);
            int num = 2017;
            List<int> buffer = new List<int>(num);
            buffer.Add(0);
            int position = 0;
            for (var i = 1; i <= num; i++)
            {
                position = ((position + spin) % i) + 1; //+ 1: current + 1 to move to the new inserted value
                buffer.Insert(position, i);                
                //Console.WriteLine(String.Join(" ", buffer.Select((n, p) => { var t = n.ToString(); if (p == position) t = $"({n})"; return t.ToString(); })));
            }
            
            return buffer.ElementAt(position + 1).ToString();
        }

...
However, you should instead use the standard list size of 256 (with values 0 to 255) and the sequence of lengths in your puzzle input. Once this process is complete, what is the result of multiplying the first two numbers in the list?
        ...
        */
        const int numbers = 256; //5

        public override string Output(string input)
        {
            /*
            input = "3,4,1,5"; 2 1 0 3 4 > 4 3 0 1 2 > 4 3 0 1 2 > 3 4 2 1 0
            */
            var list = Enumerable.Range(0, numbers).ToArray();
            int position = 0;
            int skip = 0;
            IEnumerable<int> lengths = input.Split(',').Select(_ => int.Parse(_));
            ...
            foreach (var length in lengths)
            {
                ...
                Console.WriteLine(String.Join(" ", list.Select((n, p) => { var t = n.ToString(); if (p == position) t = $"[{n}]"; return t.ToString(); })));
                position = (position + length + skip) % numbers;
                skip++;
            }


            return list.Take(2).Multiply().ToString();
        }
...
So, if your first three numbers are 64, 7, 255, they correspond to the hexadecimal numbers 40, 07, ff, and so the first six characters of the hash would be 4007ff. Because every Knot Hash is sixteen such numbers, the hexadecimal representation is always 32 hexadecimal digits (0-f) long.
...
Ignore any leading or trailing whitespace you might encounter.
        */
        ...
        {
            return input.KnotHash(); // Extensions/KnotHash
        }
    }
}
